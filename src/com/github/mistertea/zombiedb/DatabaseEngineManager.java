...
/**
 * The Class DatabaseEngineManager supports primary keys without secondary keys.
 * See {@link IndexedDatabaseEngineManager} if you want secondary keys.
 */
public class DatabaseEngineManager extends AbstractDatabaseEngineManager {
   ...
   */
  public DatabaseEngineManager(DatabaseEngine databaseEngine)
      throws IOException {
   ...
   */
  @Override
  public synchronized <F extends TFieldIdEnum, T extends TBase<?, F>> int size(
      Class<T> in) throws IOException {
   ...
   */
  @Override
  public synchronized <F extends TFieldIdEnum, T extends TBase<?, F>> void upsert(
      T thrift) throws IOException {
    try {
      String id = (String) thrift.getFieldValue(thrift.fieldForId(1));

      databaseEngine.putBytesAtomic(thrift.getClass().getSimpleName(), id,
  ...
  @Override
  protected synchronized <F extends TFieldIdEnum, T extends TBase<?, F>> void upsertNoPrimaryLock(
  ...
  public synchronized <F extends TFieldIdEnum, T extends TBase<?, F>> void upsertNonAtomic(
    ...
    try {
      ...
      databaseEngine.putBytesBatch(thrift.getClass().getSimpleName(), id,
          serializer.serialize(thrift));
    } catch (Exception e) {
      throw new IOException(e);
    }
