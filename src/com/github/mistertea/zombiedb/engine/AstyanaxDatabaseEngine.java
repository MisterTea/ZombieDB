...
public class AstyanaxDatabaseEngine implements DatabaseEngine {
	...
	public class ThriftWrapperCassandraIterator implements Iterator<byte[]> {
		...
		public ThriftWrapperCassandraIterator(
				Iterator<Row<String, String>> rowIterator) {
		...
		/*
		 * We are going to hell for this, but hasNext needs to actually do a
		 * fetch to make sure there is a next
		 */
		@Override
			...
			if (nextObject == null) {
				if (hasNext() == false) {
					throw new IllegalStateException(
							"Tried to get next when no next was there");
				}
			}
			if (nextObject == null) {
				throw new IllegalStateException(
	...
	public AstyanaxDatabaseEngine(String clusterName, String dbName,
			boolean wipe) throws IOException {
		super();
		...
		context = new AstyanaxContext.Builder()
				...
				.withAstyanaxConfiguration(
						new AstyanaxConfigurationImpl()
								...
								.setRetryPolicy(
										new BoundedExponentialBackoff(250,
												5000, 10)))
				.withConnectionPoolConfiguration(
						new ConnectionPoolConfigurationImpl("MyConnectionPool")
								...
								.setRetryBackoffStrategy(
										new FixedRetryBackoffStrategy(1000,
												1000)))
	...
	private void createDatabase() throws IOException {
		...
		try {
			cluster.addKeyspace(cluster.makeKeyspaceDefinition()
					.setName(dbName).setStrategyClass("SimpleStrategy")
					.setStrategyOptions(optsMap));
	...
	public synchronized void deleteKey(String family, String key)
			throws IOException {
		try {
			keySpace.prepareColumnMutation(getOrCreateColumnFamily(family),
					key, "Data").deleteColumn().execute();
		} catch (ConnectionException e) {
			throw new IOException(e);
		}
	}
	...
	public Set<String> getAllIds(String family) throws IOException {
		...
		try {
			result = keySpace.prepareQuery(getOrCreateColumnFamily(family))
					.getAllRows().setExceptionCallback(new ExceptionCallback() {
						@Override
	...
	public synchronized void putBytesBatch(String className, String key,
		...
		mutationBatch.withRow(getOrCreateColumnFamily(className), key)
				.putColumn("Data", value);
	}
	...
	public void putBytesAtomic(String family, String key, byte[] value)
		...
		try {
			keySpace.prepareColumnMutation(getOrCreateColumnFamily(family),
					key, "Data").putValue(value, null).execute();
		} catch (ConnectionException e) {
			throw new IOException(e);
		}
	}
	...
	public synchronized void releaseLock(String className, String key)
		...
		ColumnPrefixDistributedRowLock<String> lock = locks
				.get(new FamilyRowPair(className, key));
		if (lock == null) {
